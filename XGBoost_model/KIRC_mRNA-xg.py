...
class GV:
    ...
    def xg_find_base(self, scoring, data_x, data_y, model_xg, params, overfit=None):
        ...
        params_test1 = {"learning_rate": [0.1,0.13,0.15,0.2]}
        clf = GridSearchCV(model_xg, params_test1, cv=kfold, n_jobs=-1, scoring=scoring)
        clf.fit(data_x, data_y)
        ...
        params_test1 = {"colsample_bytree": np.arange(0.5, 0.9, 0.1), 'subsample': np.arange(0.5, 0.9, 0.1)}
        clf = GridSearchCV(model_xg, params_test1, cv=kfold, n_jobs=-1, scoring=scoring)
        clf.fit(data_x, data_y)
        ...
        params_test1 = {"gamma": np.arange(0, 1, 0.1)}
        clf = GridSearchCV(model_xg, params_test1, cv=kfold, n_jobs=-1, scoring=scoring)
        clf.fit(data_x, data_y)
        ...
        params_test1 = {"colsample_bynode": np.arange(0.4, 1, 0.1)}
        clf = GridSearchCV(model_xg, params_test1, cv=kfold, n_jobs=-1, scoring=scoring)
        clf.fit(data_x, data_y)
        ...
        params_test1 = {'reg_lambda': np.arange(1, 1.6, 0.2), 'reg_alpha': np.arange(0, 1.6, 0.2)}
        clf = GridSearchCV(model_xg, params_test1, cv=kfold, n_jobs=-1, scoring=scoring)
        clf.fit(data_x, data_y)
...
data = pd.read_table("KIRC_mrna.txt")
