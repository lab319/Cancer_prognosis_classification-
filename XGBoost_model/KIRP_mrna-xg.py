...
class GV:
    ...
    def xg_find_base(self, scoring, data_x, data_y, model_xg, params, overfit=None):
        ...
        params_test1 = {"colsample_bylevel": np.arange(0.3, 0.9, 0.1), "colsample_bynode": np.arange(0.3, 1, 0.1)}
        clf = GridSearchCV(model_xg, params_test1, cv=kfold, n_jobs=-1, scoring=scoring)
        clf.fit(data_x, data_y)
...
data = pd.read_table("KIRP_mrna.txt")
...
for train_ind, test_ind in kf.split(data, label):
    ...
    gv = GV()
    params = {}
    model, params = gv.xg_find_base('roc_auc', train_data, train_y, model, params)
    early_stop = 50
    verbose_eval = 20
    num_rounds = 450
