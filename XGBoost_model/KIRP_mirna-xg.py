...
class GV:
    ...
    def xg_find_base(self, scoring, data_x, data_y, model_xg, params, overfit=None):
        ...
        params_test1 = {"max_depth": np.arange(3, 5, 1)}
        ...
        params_test1 = {"gamma": np.arange(0, 0.9, 0.1)}
        clf = GridSearchCV(model_xg, params_test1, cv=kfold, n_jobs=-1, scoring=scoring)
        clf.fit(data_x, data_y)
        ...
        params_test1 = {'reg_lambda': np.arange(0, 0.3, 0.1), 'reg_alpha': np.arange(0, 0.3, 0.1)}
        clf = GridSearchCV(model_xg, params_test1, cv=kfold, n_jobs=-1, scoring=scoring)
        clf.fit(data_x, data_y)
...
def fpreproc(dtrain):
    ...
    print(label)
    ratio = float(np.sum(label == 1)) / len(label)
    print(ratio)
    return ratio
...
scores_3=[]
scores_4=[]
data = pd.read_table("KIRP_mirna.txt")
