...
class GV:
    def xg_find_base(self, scoring, data_x, data_y, model_xg, params, overfit=None):
        ...
        params_test1 = {"colsample_bytree": np.arange(0.3, 0.65, 0.05), 'subsample': np.arange(0.3, 0.65, 0.05)}
        clf = GridSearchCV(model_xg, params_test1, cv=kfold, n_jobs=-1, scoring=scoring)
        clf.fit(data_x, data_y)
        ...
        print("clf.best_score_", clf.best_score_)
        params_test1={"colsample_bylevel":np.arange(0.3,0.65,0.05)}
        clf = GridSearchCV(model_xg,params_test1,cv=kfold,n_jobs=-1,scoring=scoring)
        clf.fit(data_x,data_y)
        ...
        params_test1 = {"gamma": np.arange(1, 1.8, 0.1)}
        clf = GridSearchCV(model_xg, params_test1, cv=kfold, n_jobs=-1, scoring=scoring)
        clf.fit(data_x, data_y)
        ...
        params_test1 = {'reg_lambda': np.arange(1, 1.8, 0.1), 'reg_alpha': np.arange(1, 1.8, 0.1)}
        clf = GridSearchCV(model_xg, params_test1, cv=kfold, n_jobs=-1, scoring=scoring)
        clf.fit(data_x, data_y)
...
scores_5=[]
scores_6=[]
data = pd.read_table("KIRC_mirna.txt")
...
for train_ind, test_ind in kf.split(data, label):
    ...
    y_pre = model.predict(d_valid,ntree_limit=model.best_ntree_limit).reshape([-1,1])
    mms = Binarizer(0.5)
